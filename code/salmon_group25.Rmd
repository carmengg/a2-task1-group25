...
```{r, tab 3, fig.align='center',fig.cap=}
fish_annual <- fish %>% 
  index_by(year = ~year(.)) %>% 
  group_by(fish_type) %>% 
...
ggplot(data = fish_annual,aes(x=year,y=annual_count, color=fish_type))+
  ...
  labs(x= "Year", y= "Annual count", title = "Annual counts of Coho, Jack Coho, and Steelhead fish", subtitle = "surveyed from 2001-2010", color= "Salmon Species")
